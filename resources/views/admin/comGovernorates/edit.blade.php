...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.comGovernorate.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.com-governorates.update", [$comGovernorate->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('country') ? 'is-invalid' : '' }}" name="country_id" id="country_id" required>
                    @foreach($countries as $id => $entry)
                        <option value="{{ $id }}" {{ (old('country_id') ? old('country_id') : $comGovernorate->country->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('governorate_name_a') ? 'is-invalid' : '' }}" type="text" name="governorate_name_a" id="governorate_name_a" value="{{ old('governorate_name_a', $comGovernorate->governorate_name_a) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('governorate_name_l') ? 'is-invalid' : '' }}" type="text" name="governorate_name_l" id="governorate_name_l" value="{{ old('governorate_name_l', $comGovernorate->governorate_name_l) }}">
