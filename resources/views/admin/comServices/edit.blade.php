...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.comService.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.com-services.update", [$comService->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="service_name_a">{{ trans('cruds.comService.fields.service_name_a') }}</label>
                <input class="form-control {{ $errors->has('service_name_a') ? 'is-invalid' : '' }}" type="text" name="service_name_a" id="service_name_a" value="{{ old('service_name_a', $comService->service_name_a) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('service_name_l') ? 'is-invalid' : '' }}" type="text" name="service_name_l" id="service_name_l" value="{{ old('service_name_l', $comService->service_name_l) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('account') ? 'is-invalid' : '' }}" name="account_id" id="account_id">
                    @foreach($accounts as $id => $entry)
                        <option value="{{ $id }}" {{ (old('account_id') ? old('account_id') : $comService->account->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
