    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.fee-types.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('fee_type_name_a') ? 'is-invalid' : '' }}" type="text" name="fee_type_name_a" id="fee_type_name_a" value="{{ old('fee_type_name_a', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('fee_type_name_l') ? 'is-invalid' : '' }}" type="text" name="fee_type_name_l" id="fee_type_name_l" value="{{ old('fee_type_name_l', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('fee_class') ? 'is-invalid' : '' }}" name="fee_class_id" id="fee_class_id" required>
                    @foreach($fee_classes as $id => $entry)
                        <option value="{{ $id }}" {{ old('fee_class_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('fee_type_amt') ? 'is-invalid' : '' }}" type="number" name="fee_type_amt" id="fee_type_amt" value="{{ old('fee_type_amt', '') }}" step="0.0001">
