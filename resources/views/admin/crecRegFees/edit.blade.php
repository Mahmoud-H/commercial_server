...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.crecRegFee.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.crec-reg-fees.update", [$crecRegFee->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('crec_reg_request') ? 'is-invalid' : '' }}" name="crec_reg_request_id" id="crec_reg_request_id" required>
                    @foreach($crec_reg_requests as $id => $entry)
                        <option value="{{ $id }}" {{ (old('crec_reg_request_id') ? old('crec_reg_request_id') : $crecRegFee->crec_reg_request->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('fee_type') ? 'is-invalid' : '' }}" name="fee_type_id" id="fee_type_id" required>
                    @foreach($fee_types as $id => $entry)
                        <option value="{{ $id }}" {{ (old('fee_type_id') ? old('fee_type_id') : $crecRegFee->fee_type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('fee_type_amt') ? 'is-invalid' : '' }}" type="number" name="fee_type_amt" id="fee_type_amt" value="{{ old('fee_type_amt', $crecRegFee->fee_type_amt) }}" step="0.001" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('fee_qty') ? 'is-invalid' : '' }}" type="number" name="fee_qty" id="fee_qty" value="{{ old('fee_qty', $crecRegFee->fee_qty) }}" step="1" required>
